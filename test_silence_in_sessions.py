...
from utils import pathcat, find_modes, pickleData, com, get_shift_and_flow, normalize_sparse_array
...
class test_silence_in_sessions:
  ...
  def __init__(self,basePath,mouse,nSes,n_processes=0,session_classification=None,dataSet='OnACID',s_corr_min=0.3):
...
class plot_test_undetected:
  ...
  def plot(self,SNR_thr=2,rval_thr=0,CNN_thr=0.6,pm_thr=0.5,sv=False):
    ...
    assignments_OnACID = dt['assignment']
    ...
    if True:
        ...
        ax_ROIs = plt.axes([0.025,0.625,0.225,0.35])
        ax_ROIs2 = plt.axes([0.25,0.625,0.225,0.35])
        ...
        ax = plt.axes([0.52,0.65,0.2,0.325])
        ...
        ax.legend(loc='upper left',bbox_to_anchor=[-0.1,0.95],fontsize=10)

        ax = plt.axes([0.52,0.125,0.2,0.325])
        ...
        ax = plt.axes([0.85,0.85,0.125,0.125])
        ...
        ax = plt.axes([0.85,0.625,0.125,0.125])
        ...
        ax = plt.axes([0.85,0.35,0.125,0.125])
        ...
        ax = plt.axes([0.85,0.125,0.125,0.125])
        ...
        ax_A = plt.axes([0.025,0.375,0.15,0.2])
        ...
        ax_A = plt.axes([0.025,0.125,0.15,0.2])
    ...
    ax_oc = plt.axes([0.2,0.1,0.45,0.625])
    ax_oc2 = ax_oc.twinx()
    ...
    ax = plt.axes([0.65,0.1,0.175,0.625])
    ...
    ax.plot((idx_pm & idxes)[self.cluster.stats['cluster_bool'],:].sum(1),np.linspace(1,nC_clean,nC_clean),'ko',markersize=0.5)
    ax.invert_yaxis()
    ax.set_ylim([nC_clean,0])
    ax.set_yticks([])
    ax.set_xlabel('occ.')

    ax = plt.axes([0.65,0.725,0.175,0.2])
    ...
    ax = plt.axes([0.2,0.725,0.45,0.2])
    ...
    ax.plot(np.linspace(0,nS,nS),(idx_pm&idxes&idx_prev)[self.cluster.stats['cluster_bool'],:].sum(0),'^',color='tab:green',markersize=3,label='initial')
    ax.plot(np.linspace(0,nS,nS),idx_pm[self.cluster.stats['cluster_bool'],:].sum(0),'^',color='tab:orange',markersize=3,label='inferred')
    ax.plot(np.linspace(0,nS,nS),(idx_pm & idxes)[self.cluster.stats['cluster_bool'],:].sum(0),'k^',markersize=3,label='thresholded')
    ax.set_xlim([0,nS])
    ax.set_ylim([0,3300])
    ax.set_xticks([])
    ax.set_ylabel('# neurons')
    ax.spines['top'].set_visible(False)
    ax.legend(fontsize=10,bbox_to_anchor=[1.1,0.6],loc='lower right')
